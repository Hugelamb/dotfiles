...
function P.get_custom_date_fmt(args, snip, old_state, fmt)
  local fmt = fmt or "%Y-%m-%d"
  return sn(nil, i(1, os.date(fmt)))
end

function P.get_ISO_8601_date()
  return os.date("%Y-%m-%d")
end

function P.get_visual(_, parent)
...
function P.get_capture(_, snip, user_arg1, user_arg2, user_arg3)
  -- define args
  idx = user_arg1 or 1
  pre = user_arg2 or ""
  post = user_arg3 or ""
...
function P.return_filename()     -- print current working file name (excluding suffix filetype)
  return vim.fn.fnamemodify(vim.fn.expand('%'),':t')
end
function P.return_pwd()       -- print current working directory
  return vim.fn.getcwd()
end
-- dynamic node functionns from https://github.com/L3MON4D3/LuaSnip/wiki/Misc#dynamicnode-with-user-input
function P.find_dynamic_node(node)
...
function P.dynamic_node_external_update(func_indx)
