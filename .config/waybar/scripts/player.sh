...
icons=("<span_color='#8c57ff' >󰎇</span>" "<span color='#F30131' >󰗃</span>" "" "" "" "󱒀")
...
if [ "$player_status" = "Playing" -o "$player_status" = "Paused" ]; then
  ...
  info="${icon}" # $(playerctl metadata artist) - $(playerctl metadata title) "
  time="$(get_time)"
